import * as React from "react"
import { Link } from "gatsby"


const SaberMas = () => {
	return(
	<span>
		<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-arrow-right-circle" viewBox="0 0 16 16">
		  <path fillRule="evenodd" d="M1 8a7 7 0 1 0 14 0A7 7 0 0 0 1 8zm15 0A8 8 0 1 1 0 8a8 8 0 0 1 16 0zM4.5 7.5a.5.5 0 0 0 0 1h5.793l-2.147 2.146a.5.5 0 0 0 .708.708l3-3a.5.5 0 0 0 0-.708l-3-3a.5.5 0 1 0-.708.708L10.293 7.5H4.5z"/>
		</svg>
	</span>
	)
}

export const VerCodigo = () => {
	return(
	<span>
		<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-github" viewBox="0 0 16 16">
		  <path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.012 8.012 0 0 0 16 8c0-4.42-3.58-8-8-8z"/>
		</svg>
	</span>
	)
}

export const VerProyecto = () => {
	return(
	<span>
		<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-arrow-up-right-circle" viewBox="0 0 16 16">
		  <path fillRule="evenodd" d="M1 8a7 7 0 1 0 14 0A7 7 0 0 0 1 8zm15 0A8 8 0 1 1 0 8a8 8 0 0 1 16 0zM5.854 10.803a.5.5 0 1 1-.708-.707L9.243 6H6.475a.5.5 0 1 1 0-1h3.975a.5.5 0 0 1 .5.5v3.975a.5.5 0 1 1-1 0V6.707l-4.096 4.096z"/>
		</svg>
	</span>
	)
}

const Options = () => {
	return(
		<span className="flex space-x-1 absolute top-[12px] left-[10px]">
            <svg width="10" height="10">
                <circle cx="5" cy="5" r="5" fill="#FF5F57"></circle>
            </svg>
            <svg width="10" height="10">
                <circle cx="5" cy="5" r="5" fill="#FFBD2E"></circle>
            </svg>
            <svg width="10" height="10">
                <circle cx="5" cy="5" r="5" fill="#28CA41"></circle>
            </svg>
        </span>
	)
}

export const ProjectCard = ({project}) => {
	const [active, setActive] = React.useState(false)
	
	const handleHover = () => {
		setActive(true)
	}
	const handleBlur = () => {
		setActive(false)
	}
	
	const height = active ? 'h-[100%]' : 'h-[0]'
	
	return(
		<div onMouseLeave={ handleBlur }  className="rounded-md border border-gray-200 overflow-hidden shadow-md relative">
			<div className="bg-[#222222] relative p-[5px]">
				<Options />
				<h3 className="font-bold text-[14px] text-[#EEE] text-center">{project.title}</h3>
			</div>
			<img onMouseEnter={ handleHover } src={project.miniatura.url} alt={project.title} className="w-full h-[300px] max-h-[300px] object-cover"/>
			<div className={`${height} transition-[all] duration-500 ease-in-out overflow-hidden w-full absolute top-[30px] left-0`}>
				<div className="h-[100%] flex flex-col justify-center items-center space-y-4 bg-black bg-opacity-80 text-white font-bold">
					{
						project.repositorio && 
						<a href={project.repositorio} target="_blank" rel="noreferrer" className="hover:text-blue-500 flex space-x-4 items-center">
							<span>Ver codigo</span>
							<VerCodigo />
						</a>
						}
					{
						project.preview && 
						<a href={project.preview} target="_blank" rel="noreferrer" className="hover:text-blue-500 flex space-x-4 items-center">
							<span>Ver proyecto</span>
							<VerProyecto /> 
						</a>
					}
					<Link to={`/portfolio/${project.slug}`} className="hover:text-blue-500 flex space-x-4 items-center">
						<span>Saber mas </span>
						<SaberMas /> 
					</Link>
				</div>
			</div>
		</div>
	)
}
